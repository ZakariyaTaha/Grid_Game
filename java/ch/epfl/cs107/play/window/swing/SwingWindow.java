...
public final class SwingWindow extends Node implements Window {
	...
	public void update() {
		...
		final Transform projection = new Transform(width / halfX, 0.0f, 0.5f * width, 0.0f, -height / halfY,
				0.5f * height);
		...
		// Set view transform
		final AffineTransform affine = new AffineTransform(transform.m00, transform.m10, transform.m01, transform.m11,
	...
	public void drawText(String text, float fontSize, Transform transform, Color fillColor, Color outlineColor,
		...
		if (text == null || fontSize <= 0.0f || alpha <= 0.0f
			...
			return;
		gItems.add(new TextItem(text, fontSize, transform, fillColor, outlineColor, thickness, bold, italics, anchor,
				hAlign, vAlign, depth, alpha));
	}
	...
	@Override
	public void playSound(Sound sound, boolean randomFirstStart, float volume, boolean fadeIn, boolean loop,
