...
public final class SoundItem implements LineListener {
	...
	/// currentVolume float value in percent : 0.0f no currentVolume, 1.0f full
	/// currentVolume
	 ...
	 * 
	 * @param loop             (boolean): indicate if the item restart on
	 *                         self-ending
	...
	/**
	 * Apply gain currentVolume modification before starting a clip - Notice the
	 * MASTER_GAIN FloatControl value is in decibels, meaning it's a logarithmic
	 * scale, not a linear one.
	 */
	private void applyVolume() {
		// TODO the FloatControl may be problematic following the JDK version: figure it
		// out exactly
	...
	/**
	 * Update the volume using the fade step concept Notice: we finish the song if
	 * the faded out terminated
	 */
