...
public class ImageGraphics extends Node implements Graphics {
	...
	/// Anchor of the image (i.e. if the origin of the image is not the origin of
	/// the parent)
	private Vector anchor;
	/// Transparency of the image. Between 0 (invisible) amd 1 (opaque)
	private float alpha;
	/// Depth used as render priority. It is the third axis. See it as altitude :
	 ...
	 * @param roi              (RegionOfInterest): region of interest as a rectangle
	 *                         in the image
	 * @param anchor           (Vector): image anchor, not null
	 * @param alpha            (float): transparency, between 0 (invisible) and 1
	 *                         (opaque)
	 * @param depth            (float): render priority, lower-values drawn first
	 * @param removeBackground (boolean): indicate if we need to remove the uniform
	 *                         color background before using this image
	 */
	public ImageGraphics(String name, float width, float height, RegionOfInterest roi, Vector anchor, float alpha,
			float depth, boolean removeBackground) {
		this.name = name;
		this.width = width;
		this.height = height;
		this.roi = roi;
	 ...
	 * @param width  (float): actual image width, before transformation
	 * @param height (float): actual image height, before transformation
	 * @param roi    (RegionOfInterest): region of interest as a rectangle in the
	 *               image
	 * @param anchor (Vector): image anchor, not null
	...
	public void draw(Canvas canvas) {
		...
		Transform transform = Transform.I.scaled(width, height).translated(anchor.x, anchor.y)
