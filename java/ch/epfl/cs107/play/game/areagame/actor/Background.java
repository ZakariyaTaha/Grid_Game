...
public class Background extends Entity {
	...
	/**
	 * Default Background Constructor by default the Background image is using the
	...
	/**
	 * Extended Background Constructor by default the Background image is using the
	 * area title as file name
	 * 
	 * @param area   (Area): ownerArea. Not null
	...
	public Background(Area area, RegionOfInterest region) {
		super(DiscreteCoordinates.ORIGIN.toVector());
		sprite = new ImageGraphics(ResourcePath.getBackgrounds(area.getTitle()), area.getWidth(), area.getHeight(),
				region, Vector.ZERO, 1.0f, -Float.MAX_VALUE);
		sprite.setParent(this);
	}
	 ...
	 * @param region (RegionOfInterest): region of interest in the image for the
	 *               background, may be null
	 * @param name   (String): Background file name (i.e only the name, with neither
	 *               path, nor file extension). Not null
	 */
	public Background(Area area, RegionOfInterest region, String name) {
		super(DiscreteCoordinates.ORIGIN.toVector());
		sprite = new ImageGraphics(ResourcePath.getBackgrounds(name), area.getWidth(), area.getHeight(), region,
	...
	public Background(String name, int width, int height, RegionOfInterest region) {
		super(DiscreteCoordinates.ORIGIN.toVector());
		sprite = new ImageGraphics(ResourcePath.getBackgrounds(name), width, height, region, Vector.ZERO, 1.0f,
				-Float.MAX_VALUE);
		sprite.setParent(this);
	}
