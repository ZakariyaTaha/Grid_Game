...
public class Dialog implements Graphics {
	...
	private static final int MAX_LINE_SIZE = 85;
	 ...
	 * @param backgroundName (String): Background file name (i.e only the name, with
	 *                       neither path, nor file extension), may be null
	 * @param area           (Area): this owner area to compute scale factor ratios,
	 *                       not null
	 * 
	 */
	public Dialog(String text, String backgroundName, Area area) {
		...
		final float fontSize = 0.025f * area.getCameraScaleFactor();
